...
namespace zarkiaclb
{
    public class clsSalInsurenceDisk
    {
        ...
        /// <returns></returns>
        public bool CreateDBFPersonal(List<SalInsurenceDiskPersonal> insList, string DBFPath)
        {
            ...
            {
                throw new Exception("امکان ساخت دیسکت برای کارمندان وجود ندارد ، یک فایل با همین نام در مسیر مورد وجود دارد که باز است و امکان دسترسی به آن وجود ندارد ، لطفا آن فایل را ببندید یا آن را به مسیر دیگیری منتقل نمایید.");
        ...
        /// <param name="DBFName"></param>
        /// <returns></returns>
        public bool CreateDBFFactory(SalInsurenceDiskFactory insList, string DBFPath)
        {
            string DBFName = "DSKKAR00.dbf";

            //if (!UniCodeChecker())
            //{ ShowAthenticationError(); return false; }
            //try
            //{
            //    AuthEntities db = new AuthEntities(conn);
            //    tblSalInsurenceDiskFactory tbl = new tblSalInsurenceDiskFactory()
            //    {
            //        DSK_ADRS=insList.DSK_ADRS,
            //        DSK_FARM=insList.DSK_FARM,
            //        DSK_NAME=insList.DSK_NAME,
            //        DSK_ID=insList.DSK_ID,
            //    };

            //    db.tblSalInsurenceDiskFactories.Add(tbl);
            //    db.SaveChanges();
            //}
            ...
            {
                throw new Exception("امکان ساخت دیسکت برای کارخانه وجود ندارد ، یک فایل با همین نام در مسیر مورد وجود دارد که باز است و امکان دسترسی به آن وجود ندارد ، لطفا آن فایل را ببندید یا آن را به مسیر دیگری منتقل نمایید.");
                //return false;
            }
            return true;
        }
        ...
        /// <returns></returns>
        public string Convert_Unicode_To_IranSystem(string Unicode_Text)
        {
