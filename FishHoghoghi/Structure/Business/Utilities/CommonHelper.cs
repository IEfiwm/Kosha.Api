...
using FishHoghoghi.Utilities.Defines;
...
namespace FishHoghoghi.Utilities
{
    public static partial class CommonHelper
    {
        ...
        {
            return obj == null ? default : Deserialize<T>(Serialize(obj));
        }
        ...
        {
            return input.ToString().ConvertToPersianNumber();
        }
        ...
        {
            return value.ToggleStringAtStart("/", shouldHave);
        }
        ...
        [Obsolete("using IsIranianNationalIdValid")]
        public static bool IsNationalIdValid(string nationalId)
        {
            //http://www.dotnettips.info/post/1097/
            //input has 11 digits that all of them are not equal
            return true;

            if (!Regex.IsMatch(nationalId, @"^(?!(\d)\1{10})\d{11}$"))
            {
                return false;
            }
            ...
            int dec = Convert.ToInt32(nationalId.Substring(9, 1)) + 2;

            int[] coef = { 29, 27, 23, 19, 17, 29, 27, 23, 19, 17 };

            var sum = Enumerable.Range(0, 10)
                .Select(x => (Convert.ToInt32(nationalId.Substring(x, 1)) + dec) * coef[x])
                .Sum() % 11;
        ...
        public static DateTime? FromUnixTimeSeconds(this long? seconds, bool localTime)
        {
            ...
            return seconds.Value.FromUnixTimeSeconds(localTime);
        }
        ...
        public static T GetValueFromDisplayName<T>(string name)
        {
            ...
            {
                var attribute = System.Attribute.GetCustomAttribute(field, typeof(DisplayAttribute)) as DisplayAttribute;
                if (attribute != null)
                {
        ...
        public static bool IsAfterDate(int year, int month)
        {
            if (year < 1398 || (year == 1398 && month < 8))
            {
                return false;
            }
        ...
        public static bool IsAfterDate(int year, int month, int indexYear, int indexMonth)
        {
            if (year < indexYear || (year == indexYear && month < indexMonth))
            {
                return false;
            }
        ...
        public static bool IsBeforeCurrentDate(int year, int month)
        {
            var date = DateTime.Now.ToPersianDateTime();

            //if (year * 366 + (month + 1) * 32 < date.Year * 366 + date.Month * 32)
            //{
            //    return true;
            //}
        ...
        public static string ReadJson(string location, string fileName)
        {
            string jsonResult;

            string path = HttpContext.Current.Server.MapPath(String.Format("~/App_Data/{0}/{1}", location, fileName));
        ...
        public static string GetMonthLabel(string str)
        {
            switch (str)
            {
                case "1":
